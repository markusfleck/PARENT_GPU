//    A program to extract the entropy and mutual information terms from the
//    binary output of the PARENT program Copyright (C) 2015  Markus Fleck
//    (member of the laboratory of Bojan Zagrovic, University of Vienna)
//
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (int k = 1; k <= nBonds; k++) {
      if (longOutput) {
        cout << "bond " << k << "   " << mat.getEntropy(TYPE_B, k)
             << endl; // write the 1D entropies to stdout  and keep track of the
                      // total sums
      }
    ...
    // then for every bond-bond pair get the mutual information and write it as
    // well as the 2D entropy to stdout
    for (int i = 1; i <= nBonds; i++) {
      for (int j = i + 1; j <= nBonds; j++) {
        ...
        if (longOutput) {
          cout << "bond-bond " << i << "  " << j << "   " << entropy2D << "   "
               << mutual << endl;
        }
        totalBbEntropy +=
            entropy2D; // also keep track of the total bond-bond enropy sum
        totalBbMutual += mutual; // and the bond-bond mutual information sum
      }
    }

    for (int i = 1; i <= nBonds; i++) { // same for all bond-angle pairs, except
                                        // that the array is in forward order
      for (int j = 1; j <= nAngles; j++) {
        mutual = mat.getMutual(TYPE_B, TYPE_A, i, j);
        entropy2D = mat.get2DEntropy(TYPE_B, TYPE_A, i, j);
        if (longOutput) {
          cout << "bond-angle " << i << "  " << j << "   " << entropy2D << "   "
               << mutual << endl;
        }
    ...
    for (int i = 1; i <= nBonds; i++) { // bond-dihedral pairs (forward order)
      for (int j = 1; j <= nDihedrals; j++) {
        ...
        if (longOutput) {
          cout << "bond-dihedral " << i << "  " << j << "   " << entropy2D
               << "   " << mutual << endl;
        }
    ...
    for (int i = 1; i <= nAngles; i++) { // angle-dihedral pairs (forward order)
      for (int j = 1; j <= nDihedrals; j++) {
        ...
        if (longOutput) {
          cout << "angle-dihedral " << i << "  " << j << "   " << entropy2D
               << "   " << mutual << endl;
        }
    ...
    cout << "TOTAL 2D BONDS-BONDS MUTUAL INFORMATION = " << totalBbMutual
         << endl;
    // cout<<"TOTAL 2D BONDS-ANGLES ENTROPY = "<<totalBaEntropy<<endl;
    cout << "TOTAL 2D BONDS-ANGLES MUTUAL INFORMATION = " << totalBaMutual
         << endl;
    // cout<<"TOTAL 2D BONDS-DIHEDRALS ENTROPY = "<<totalBdEntropy<<endl;
    cout << "TOTAL 2D BONDS-DIHEDRALS MUTUAL INFORMATION = " << totalBdMutual
         << endl;
    // cout<<"TOTAL 2D ANGLES-ANGLES ENTROPY = "<<totalAaEntropy<<endl;
    cout << "TOTAL 2D ANGLES-ANGLES MUTUAL INFORMATION = " << totalAaMutual
         << endl;
    // cout<<"TOTAL 2D ANGLES-DIHEDRALS ENTROPY = "<<totalAdEntropy<<endl;
    cout << "TOTAL 2D ANGLES-DIHEDRALS MUTUAL INFORMATION = " << totalAdMutual
         << endl;
    // cout<<"TOTAL 2D DIHEDRALS-DIHEDRALS ENTROPY = "<<totalDdEntropy<<endl;
    cout << "TOTAL 2D DIHEDRALS-DIHEDRALS MUTUAL INFORMATION = "
         << totalDdMutual << endl;
    cout << "TOTAL CONFIGURATIONAL ENTROPY = "
         << totalBondsEntropy + totalAnglesEntropy + totalDihedralsEntropy -
                totalBbMutual - totalBaMutual - totalBdMutual - totalAaMutual -
                totalAdMutual - totalDdMutual
         << endl;
