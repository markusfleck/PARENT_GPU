//    A class for handling an EntropyMatrix from a .par file of the PARENT_GPU suite 
...
Bat::Bat(char const * infile_str){
  ...
  if(!infile.good())
    {
      My_Error my_error((string("ERROR OPENING FILE ")+string(infile_str)+string("! ABORTING.\n")).c_str());
      throw my_error;
    }
  ...
  catch(My_Error my_error)
  {
    throw my_error;
      ...
      My_Error my_error((string("ERROR WHILE READING THE HEADER OF THE FILE ")+string(infile_str)+string("! ABORTING.")).c_str());
      throw my_error;    
  }
...
void Bat::read_BAT_header() {
    ...
    infile.read((char*)&precision, sizeof(int)); //then read an integer declaring if the trajectory is stored in double precision
    fail=fail | (infile.rdstate() & std::ifstream::failbit);
    infile.read((char*)&n_dihedrals,sizeof(int));//read an integer containing the number of dihedrals
    fail=fail | (infile.rdstate() & std::ifstream::failbit);
    infile.read((char*)&n_frames,sizeof(int)); //and an integer containing the number of frames
    fail=fail | (infile.rdstate() & std::ifstream::failbit);
    ...
    if((precision!=0) && (precision!=1)) {
        ostringstream oss;
        oss <<"ERROR: FILE HEADER CORRUPTED. DOUBLE PRECISION VALUE ("<<precision<<") NEITHER 0 NOR 1!";
        My_Error my_error(oss.str());
        throw my_error;
    }
    if(n_dihedrals>19997) {
        cerr << "WARNING: "<<n_dihedrals+3<<" ATOMS DECLARED IN THE FILE HEADER (CORRUPTED?). THIS WILL LEAD TO LARGE OUTPUT."<<endl;
    }
    if(n_dihedrals<0) {
        ostringstream oss;
        oss <<"ERROR: FILE HEADER CORRUPTED. NUMBER OF DIHEDRALS ("<<n_dihedrals<<") < 0!";
        My_Error my_error(oss.str());
        throw my_error;
    }
    if(n_frames<1) {
        ostringstream oss;
        oss <<"ERROR: FILE HEADER CORRUPTED. NUMBER OF FRAMES ("<<n_frames<<") < 1!";
        My_Error my_error(oss.str());
        throw my_error;
    }
    ...
    if (version>=3) {
        for(unsigned int i=0; i<n_dihedrals+3; i++) { //for ever atom in the system
            ...
            infile.read((char*)&(residue_numbers[i]), sizeof(float));//read the number of the residue it belongs to
            fail=fail | (infile.rdstate() & std::ifstream::failbit);
    ...
    for(unsigned int i=0; i<n_dihedrals+3; i++) { //and read the whole massestor of the atoms in the system in single precision (float)
        masses.push_back(0);
...
streamoff Bat::get_dofs_begin()
{
    return dofs_begin;
}


ifstream* Bat::get_file()
{
    return &infile;
}

unsigned int Bat::get_n_frames()
{
    return n_frames;
}

unsigned int Bat::get_n_dihedrals()
{
    return n_dihedrals;
}

unsigned int Bat::get_precision()
{
    return precision;
}
