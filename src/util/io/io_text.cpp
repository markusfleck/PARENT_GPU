...
int read_ndx_file(ifstream *ndxfile, vector<int> *group1, vector<int> *group2,
  ...
  while (getline((*ndxfile), line)) { // for all lines in the file
    compressedLine = delete_char(
        line, ' '); // if the line defines a group of atoms, e. g. [ Backbone ]
    if ((compressedLine[0] == '[') &&
        (compressedLine[compressedLine.length() - 1] == ']')) {
      group1flag = 0;
      group2flag = 0;
    }

    if (group1flag ==
        1) { // if the group has been identified as group1 to retrieve
      linestream.clear();
      linestream << line;
      while (getline(linestream, item, ' ')) {
        if (item.length() != 0) {
          ...
          for (unsigned int i = 0; i < item.length(); i++) {
            isAtom = isAtom && isdigit(item[i]);
          }
          if (!isAtom) {
            cerr << "ERROR READING INDEX FILE: ITEM \"" << item
                 << "\" IN SECTION \"" << name1
                 << "\" IS NOT A PROPER ATOM NUMBER!" << endl;
            return 1;
          }
    ...
    if (group2flag ==
        1) { // if the group has been identified as group2 to retrieve
      linestream.clear();
      linestream << line;
      while (getline(linestream, item, ' ')) {
        if (item.length() != 0) {
    ...
    if (compressedLine == name1) {
      group1flag = 1;
    } // check if the line declares the requested group1
    if (compressedLine == name2) {
      group2flag = 1;
    } // or group2
  }
