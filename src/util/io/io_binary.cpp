...
int write_BAT_header(ofstream *outfile, int double_prec, int numframes,
  ...
  // WHEN CHANGING SOMETHING HERE, REMEMBER TO UPDATE THE LINE
  // "outfile2.seekp(3*sizeof(int), ios::beg);" IN FUNCTION convert_xtc_to_BAT()
  // file BAT_trajectory.cpp!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  (*outfile).write(
      (char *)&version,
      sizeof(int)); // first write the .bat version number as an integer
  fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
  (*outfile).write((char *)&double_prec,
  ...
  for (int i = 0; i < dummy + 3; i++) { // for all atoms in the system
    ...
    sprintf(dummystring, "%.7s",
            ((*atomNames)[i]).c_str()); // write the name of the atom
...
int read_BAT_header(ifstream *infile, int *double_prec, int *numFrames,
  ...
  (*infile).read((char *)double_prec,
                 sizeof(int)); // then read an integer declaring if the
                               // trajectory is stored in double precision
  fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
  (*infile).read(
      (char *)&nDihedrals,
  ...
  if (version < 0) {
    cerr << "ERROR: FILE HEADER CORRUPTED. VERSION NUMBER (" << version
         << ") < 0!" << endl;
    return 1;
  }
  if ((*double_prec != 0) && (*double_prec != 1)) {
    cerr << "ERROR: FILE HEADER CORRUPTED. DOUBLE PRECISION VALUE ("
         << (*double_prec) << ") NEITHER 0 NOR 1!" << endl;
    return 1;
  }
  if (nDihedrals > 19997) {
  ...
  if (nDihedrals < 0) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER OF DIHEDRALS (" << nDihedrals
         << ") < 0!" << endl;
    return 1;
  }
  if ((*numFrames) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER OF FRAMES (" << (*numFrames)
         << ") < 1!" << endl;
    return 1;
  }
  ...
  if (version >= 3) {
    for (int i = 0; i < nDihedrals + 3; i++) { // for ever atom in the system
      ...
      fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
      (*infile).read(dummystring, 8 * sizeof(char)); // read the name of the
                                                     // atom
  ...
  for (int i = 0; i < nDihedrals; i++) { // then for all dihedrals
    ...
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    (*infile).read((char *)&((*dihedrals_top)[i][1]),
                   sizeof(int)); // second atom
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    (*infile).read((char *)&((*dihedrals_top)[i][2]), sizeof(int)); // third
                                                                    // atom
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    (*infile).read((char *)&((*dihedrals_top)[i][3]),
                   sizeof(int)); // fourth atom
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
...
int write_BAT_frame(ofstream *outfile, int double_prec, int nDihedrals,
  ...
  (*outfile).write((char *)&time,
                   sizeof(float)); // write the time of the current according
                                   // .xtc frame as a float
  fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
  (*outfile).write((char *)&xtcPrec,
...
int read_BAT_frame(ifstream *infile, int precision, int nDihedrals, float *time,
                   float *xtcPrec, float **dbox, double *root_origin_cartesian,
                   double *root_origin_theta, double *root_origin_phi,
                   double *root_origin_dihedral, double *my_bonds,
                   double *my_angles, double *my_dihedrals) {
  ...
  if (precision == 1) { // if double precision is used
    ...
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    (*infile).read((char *)my_angles,
                   sizeof(double)); // and the angle between the two rootbonds
                                    // (internal coordinates)
    fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    for (int i = 0; i < nDihedrals;
         i++) { // then for all dihedrals in the system
      (*infile).read((char *)&(my_bonds[bCounter]),
                     sizeof(double)); // read the bondlength between the last
                                      // two atoms in the dihedral
      fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
      bCounter++;
      (*infile).read((char *)&(my_angles[aCounter]),
                     sizeof(double)); // read the angle between the last threee
                                      // atoms of the dihedral#
      fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
      aCounter++;
      (*infile).read((char *)&(my_dihedrals[dCounter]),
                     sizeof(double)); // and the value of the dihedral itself
      fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
      dCounter++;
    }
...
int write_PAR_header(ofstream *outfile, int nDihedrals, int double_prec,
  ...
  (*outfile).write((char *)&numFrames,
                   sizeof(int)); // and an integer containing the number of
                                 // frames of the trajectory used for calculation
  fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
  (*outfile).write(
      (char *)&bDens1D,
  ...
  for (int i = 0; i < dummy + 3; i++) { // for ever atom in the system
    ...
    (*outfile).write(dummystring,
                     8 * sizeof(char)); // write the name of the atom
    fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
    for (int j = 0; j < 31; j++) {
      dummystring[j] = '\0';
    }
  ...
  for (int i = 0; i < dummy; i++) { // then for all dihedrals
    ...
    (*outfile).write((char *)&((*dihedrals_top)[i][1]),
                     sizeof(int)); // second atom
    fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
    (*outfile).write((char *)&((*dihedrals_top)[i][2]),
                     sizeof(int)); // third atom
    fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
    (*outfile).write((char *)&((*dihedrals_top)[i][3]),
                     sizeof(int)); // fourth atom
    fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
  ...
  for (int i = 0; i < dummy + 3; i++) {
    (*outfile).write(
        (char *)&((*masses)[i]),
        sizeof(float)); // then write the whole massestor of the atoms in the
                        // system in single precision (float)
    fail = fail | ((*outfile).rdstate() & std::ofstream::failbit);
  }
  return fail; // if anything failed return a 1, otherwise a 0
...
int read_PAR_header(ifstream *infile, int *nDihedrals, int *double_prec,
  ...
  (*infile).read(
      (char *)double_prec,
      sizeof(int)); // then read an integer declaring if the trajectory was
                    // stored in double precision (-1 means unknown)
  fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
  (*infile).read(
      (char *)nDihedrals,
  ...
  if ((*double_prec != 0) && (*double_prec != 1) && (*double_prec != -1)) {
    cerr << "ERROR: FILE HEADER CORRUPTED. DOUBLE PRECISION VALUE ("
         << (*double_prec) << ") NEITHER 0, 1 NOR -1!" << endl;
    return 1;
  }
  if ((*nDihedrals) > 19997) {
    cerr << "WARNING: " << nDihedrals + 3
  ...
  if ((*nDihedrals) < 0) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER OF DIHEDRALS ("
         << (*nDihedrals) << ") < 0!" << endl;
    return 1;
  }
  if ((*numFrames) < 0) {
  ...
  if ((*bDens1D) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR BONDS IN 1D ("
         << (*bDens1D) << ") < 1!" << endl;
    return 1;
  }
  if ((*aDens1D) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR ANGLES IN 1D ("
         << (*aDens1D) << ") < 1!" << endl;
    return 1;
  }
  if ((*dDens1D) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR DIHEDRALS IN 1D ("
         << (*dDens1D) << ") < 1!" << endl;
    return 1;
  }
  if ((*bDens) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR BONDS IN 2D ("
         << (*bDens) << ") < 1!" << endl;
    return 1;
  }
  if ((*aDens) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR ANGLES IN 2D ("
         << (*aDens) << ") < 1!" << endl;
    return 1;
  }
  if ((*dDens) < 1) {
    cerr << "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR DIHEDRALS IN 2D ("
         << (*dDens) << ") < 1!" << endl;
    return 1;
  }

  if ((*version) >= 3) {
    for (int i = 0; i < (*nDihedrals) + 3; i++) { // for every atom in the
                                                  // system
      (*infile).read(
          dummystring,
          8 * sizeof(char)); // read the name of the residue it belongs to
      ...
      (*belongsToMolecule)
          .push_back(dummystring); // and the name of the molecule it belongs to
      fail = fail | ((*infile).rdstate() & std::ifstream::failbit);
    }
  }
  ...
  for (int i = 0; i < (*nDihedrals); i++) { // then for all dihedrals
    ...
    if ((*dihedrals_top)[i][5] < -1) {
      cerr << "ERROR: FILE HEADER CORRUPTED. DIHEDRAL IS PHASEANGLE OF A "
              "DIHEDRAL WITH NEGATIVE ID !"
           << endl;
      return 1;
    }
  ...
  for (int i = 0; i < (*nDihedrals) + 3; i++) {
    ...
    (*infile).read((char *)&((*masses)[i]),
                   sizeof(float)); // then read the whole massestor of the atoms
...
int write_PAR_body(ofstream *par_file, int nDihedrals, double *bondsEntropy1D,
                   double *anglesEntropy1D, double *dihedralsEntropy1D,
                   double *bbEntropy, double *baEntropy, double *bdEntropy,
                   double *aaEntropy, double *adEntropy, double *ddEntropy) {
  int fail = 0;

  int nBonds = nDihedrals + 2;
  int nAngles = nDihedrals + 1;

  (*par_file).write((char *)bondsEntropy1D,
                    nBonds *
                        sizeof(double)); // write the 1D entropy bonds array
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)anglesEntropy1D,
                    nAngles *
                        sizeof(double)); // write the 1D entropy angles array
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)dihedralsEntropy1D,
                    nDihedrals *
  ...
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)baEntropy,
                    nBonds * nAngles *
                        sizeof(double)); // write the 2D bonds-angles matrix
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)bdEntropy,
                    nBonds * nDihedrals *
  ...
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)adEntropy,
                    nAngles * nDihedrals *
                        sizeof(double)); // write the 2D angles-dihedrals matrix
  fail = fail | ((*par_file).rdstate() & std::ofstream::failbit);
  (*par_file).write((char *)ddEntropy,
...
int read_PAR_body(ifstream *par_file, int nDihedrals, double **bondsEntropy1D,
                  double **anglesEntropy1D, double **dihedralsEntropy1D,
                  double **bbEntropy, double **baEntropy, double **bdEntropy,
                  double **aaEntropy, double **adEntropy, double **ddEntropy) {
  int fail = 0;
  ...
  (*bondsEntropy1D) = (double *)malloc(
      nBonds *
      sizeof(double)); // node allocates storage for reading the .par file
  ...
  (*ddEntropy) =
      (double *)malloc(nDihedrals * (nDihedrals - 1) / 2 * sizeof(double));
  ...
  if (!((*bondsEntropy1D != NULL) && (*anglesEntropy1D != NULL) &&
    ...
    cerr << "ERROR: ALLOCATION FOR PAR FILE BODY FAILED. MORE MEMORY NEEDED?"
         << endl;
    return 2;
  }

  (*par_file).read((char *)*bondsEntropy1D,
                   nBonds * sizeof(double)); // and the actually reads it in
                                             // (see write_PAR_body for details)
